...
class SalesRepUtils:
    ...
    def _authenticate_request(self):
        ...
        # Ensure request environment is restricted to the rep's company
        company = sales_rep.company_id
        if company:
            # We set both the company and the allowed_company_ids to ensure 
            # all multi-company record rules (using 'company_ids') and 
            # environment-based logic (using 'request.env.company') are correctly scoped.
            request.env = request.env(
                user=sales_rep.user_id,
                context=dict(request.env.context, allowed_company_ids=[company.id])
            )
            _logger.info(f"Auth: Set request user to {sales_rep.user_id.name} and company to {company.name} (ID: {company.id}) for Sales Rep {sales_rep.name}")
